...
public class RPHASTMatrixAlgorithm extends AbstractMatrixAlgorithm {
    ...
    public MatrixResult compute(MatrixLocations srcData, MatrixLocations dstData, int metrics) throws Exception {
        ...
        float[] weights = null;
        List<List<EdgeDetail>> detailsMatrix = null;
        ...
        if (MatrixMetricsType.isSet(metrics, MatrixMetricsType.DETAILS))
            detailsMatrix = new ArrayList<>(tableSize);
        ...
        if (!srcData.hasValidNodes() || !dstData.hasValidNodes()) {
            ...
            pathMetricsExtractor.calcValues(originalDestTrees, srcData, dstData, times, distances, weights,  detailsMatrix);
        }
        ...
        if (MatrixMetricsType.isSet(metrics, MatrixMetricsType.DETAILS))
            mtxResult.setDetailsMatrix(detailsMatrix);
